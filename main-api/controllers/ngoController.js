...
const registerNgo = async (req, res) => {
  ...
  try {
    ...
    await aiService.writeAuditEvent(
    ...
    // === STEP 2: Read the uploaded file and call the internal AI verification service ===
    const filePath = req.file.path;
    const mimeType = req.file.mimetype;
    const fileBuffer = fs.readFileSync(filePath);
    const fileContent = fileBuffer.toString('base64');

    console.log('[Main API] Calling internal AI verification service...');

    const verificationResult = await aiService.verifyDocument(fileContent, mimeType, ngoId);
    console.log('[Main API] Received verification from AI service.');
    ...
    await aiService.writeAuditEvent(
      ...
      `ngo-${ngoId}`,
      {
        ...
        verificationDetails: verificationResult.redFlags,
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const expectedHash = crypto.createHash('sha256').update(JSON.stringify(recordToHash)).digest('hex');
